...
<div class="content-wrapper">
    ...
    <div class="content mb-5">
      <h2>Nagori</h2>
      <h6>Daftar Nagori di Kecamatan Sidamanik</h6>
      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <?php
          $no = 1;
          $sql = "SELECT id_nagori, nm_nagori, nm_pangulu FROM nagori ORDER BY id_nagori ASC";
          $result = $mysqli->query($sql);

          if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              $nm_nagori  = htmlspecialchars($row['nm_nagori'] ?? '', ENT_QUOTES, 'UTF-8');
              $nm_pangulu = htmlspecialchars($row['nm_pangulu'] ?? '', ENT_QUOTES, 'UTF-8');
            ...
            echo "<tr><td colspan='3'>Belum ada data nagori.</td></tr>";
          }
          ?>
        </tbody>
      </table>
